...
# app = Flask(__name__)
# CORS(app, resources={r"/*": {"origins": "http://172.25.50.9:3000"}})
...
# Note: The patient showed signs of catheter-related infection.
# Label: Infection

# Note: Post-operative bleeding required readmission.
# Label: Surgery

# Note: The monitor failed to record ECG.
...
#             label = decoded.split("Label:")[-1].strip().split("\n")[0].strip()

#             allowed_labels = ["Infection", "Surgery", "Equipment issue", "Other"]
...
# @app.route("/")
# def index():
#     return "Visit / to use Swagger UI (http://localhost:5088/)."
...
#         db.create_all()
#     app.run(host='0.0.0.0', port=5088, debug=True, use_reloader=False)
...
class Classify(Resource):
    ...
    def post(self):
        ...
        try:
            data = request.json
            note = data['note']
            label, raw_output = classify_text(note)
...
class Analytics(Resource):
    def get(self):
        ...
        if df['ground_truth'].notna().sum() > 0:
            ...
            cm = confusion_matrix(gt_df['ground_truth'], gt_df['label'], labels=labels)
...
@app.route("/")
def index():
    return "Visit / to use Swagger UI (http://localhost:5089/)."
...
if __name__ == '__main__':
    with app.app_context():
        db.create_all()
    app.run(host='0.0.0.0', port=5089, debug=True, use_reloader=False)
