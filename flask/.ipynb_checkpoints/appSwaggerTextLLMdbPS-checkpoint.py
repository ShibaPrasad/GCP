...
app = Flask(__name__)
CORS(app)  # Add this line after Flask() is instantiated
...
#             print("Record saved to DB with label:", label)
...
upload_parser.add_argument('file', location='files',
                           type=FileStorage, required=True, help='CSV file with MRN, Note, Department, EventDate[, GroundTruth]')
...
class Analytics(Resource):
    def get(self):
        ...
        if df['ground_truth'].notna().sum() > 0:
            ...
            cm = confusion_matrix(gt_df['ground_truth'], gt_df['label'],
...
@app.route('/')
def index():
    return "Visit / to use Swagger UI (http://localhost:5083/)."
...
if __name__ == '__main__':
    ...
    app.run(host='0.0.0.0', port=5083, debug=True, use_reloader=False)
